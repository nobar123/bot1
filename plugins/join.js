...
let handler = async(m, { conn, usedPrefix, text, isOwner, isPrems }) => {
  conn.req = conn.req || {}
  let [_, code, expired] = text.match(linkRegex) || []
  if(!code) return m.reply("Where Link grup?")
  if (!linkRegex.exec(text)) throw "Dumb Link Unvalid loll"
  if(isPrems || isOwner || m.fromMe) {
    ...
    m.reply("Link Group Sudah dikirim ke owner dan Sedang di Diperiksa...")
  }
}
...
handler.before = async function(m, {conn}) {
  ...
  if(isAccept) {
    ...
    m.reply("Ok")
    conn.req["join-" + code].msg.reply("grup ditolak oleh Developer/Pemilik untuk beberapa alasan tertentu , Jika ingin melewati pemeriksaan grup silahkan upgrade ke *Premium*")
    delete conn.req["join-" + code]
    await m.quoted.delete()
 }
}
