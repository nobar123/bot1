...
let handler = async (m, { conn }) => {
  ...
  const skinsword = db.data.users[m.sender].swordskin
 cpt = `
*[ INVENTORY  @${m.sender.split("@")[0]} (${conn.getName(m.sender)})]*

*🔑 Season Now:* ${nowS}
*😈History Season:* ${db.data.users[m.sender].season}
${readMore}
${Object.keys(inventory.others).map(v => user[v] && `*${global.rpg.emoticon(v)}${v}:* ${user[v]}`).filter(v => v).join('\n')}${tools ? `


*📍 Tools*
${skinsword ? global.rpg.emoticon("sword")+'SwordSkin: '+ skinsword : ""}
${tools}` : ''}${items ? `
...
${cooldowns}` : ''}
`.trim()
  conn.sendButton(m.chat, cpt, "Rpg-games", null, ["Season", "/season"], m, {mentions: conn.parseMention(cpt)})
}
