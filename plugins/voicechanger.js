...
let handler = async (m, { conn, args, usedPrefix, command }) => {
    try {
        ...
        if (!args[0] || !args[1]) throw 'xample:  ' + usedPrefix + command+' <tempo> <numberasetrate>'
        ...
        if (/deep/.test(command)) set = `-af atempo=4/4,asetrate=${args[0]}*2/3`
        if (/earrape/.test(command)) set = `-af volume=${args[0]}`
