...
import Canva from "discord-canvas"
    ...
    try {
        ...
        try {
            ...
            if (user) {
                ...
                //BRUH
                if (!isNumber(user.baraklv))
                user.baraklv = 1
                if (!isNumber(user.mxbarak))
                user.mxbarak = 30
                if (!('barak' in user))
                    user.kandang = false
                if (!('pasukan1' in user))
                ...
                if (!user.barak) {
                  if (!user.pasukan1){
                    if (!isNumber(user.cavalry))
                        user.cavalry = 0
                    if (!isNumber(user.cavalryexp))
                        user.cavalryexp = 0
                    if (!isNumber(user.huntingdog))
                    user.huntingdog = 0
                    if (!isNumber(user.huntingdogexp))
                    user.huntingdogexp = 0
                  }
                }
                //
                ...
                if (!('email' in user))
                    ...
                    if (!('season' in user))
                    user.season = ''
                if (!isNumber(user.tangkuning))
                    user.tangkuning = 0
               if (!isNumber(user.udang))
                    user.udang = 0
               if (!isNumber(user.queenangelfish))
                    user.queenangelfish = 0
                if (!isNumber(user.buntal))
                    user.buntal = 0
                 if (!isNumber(user.spottailpinfish))
                    user.spottailpinfish = 0
            } else
                db.data.users[m.sender] = {
                    exp: 0,
                    limit: 10,
        ...
        for (let name in plugins) {
            ...
            if ((usedPrefix = (match[0] || '')[0])) {
                ...
                if (!isPrems && plugin.limit && db.data.users[m.sender].limit < plugin.limit * 1) {
                    this.reply(m.chat, `Limit anda habis, silahkan beli melalui *${usedPrefix}buy limit jumlah* Atau bisa Membeli paket premium agar bisa menikmati fitur tanpa batas limit`, m)
...
export async function participantsUpdate({ id, participants, action }) {
            ...
            if (chat.welcome) {
                ...
                for (let user of participants) {
                    ...
                    try {
                      ...
                      let lea = await new Canva.Goodbye()
 ...
 let wel = await new Canva.Welcome()
  .setUsername(`${await this.getName(user)}`)
  ...
  .toAttachment();
                        ...
                        this.sendButton(id, text, author, action === 'add' ? wel.toBuffer() : lea.toBuffer(), [ [action == "add" ? "Selamat datang" : "Selamat tinggal", "oalah"], ["Menu", ".menu"]], false, { mentions: [user] })
