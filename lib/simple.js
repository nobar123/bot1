...
END:VCARD
    ...
    return await this.relayMessage(jid, { requestPaymentMessage }, { ...options });
}
},
copyLink: {
...
let id = await conn.relayMessage(jid, {
 ...
 sendPoll: {
async value(jid, name = "Example", polls = ["1", "2", "3"]) {
  if(!/^([A-z0-9\-]+@(s\.whatsapp\.net|g.us))$/i.test(jid)) return false
  if(typeof name != "string") name = "Example"
  if(!Array.isArray(polls)) polls = ["1", "2", "3"]
  polls = [...new Set(polls)]
  let options = polls.filter(v => typeof v == "string").map(v => ({ optionName: v }))
  if(!(options.length > 1)) options = ["1", "2", "3"].map(v => ({ optionName: v }))
