...
async function start(oldSocket = null, opts = { store }) {
    ...
    let conn = makeWASocket({
        ...
        getMessage: async (key) => (opts.store.loadMessage(/** @type {string} */(key.remoteJid), key.id) || opts.store.loadMessage(/** @type {string} */(key.id)) || {}).message || { conversation: 'Gagal Menghubungkan Server dan Memuat Pesan!, Hubungi Pemilik terkait Eror pada bot.' },
    })
